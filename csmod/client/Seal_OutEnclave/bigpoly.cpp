...
namespace seal
{
    ...
    BigPoly &BigPoly::operator =(const BigPoly& assign)
    {
        ...
        coeff_count_ = assign.coeff_count();  // ww31, add here to possibly fix a bug
 ...
 //       cout<<"In bigpoly class, assign.coeff_count() = "<<assign.coeff_count()<<endl;
 //       cout<<"In bigpoly class, coeff_count_ = "<<coeff_count_<<endl;
        return *this;
    }
    ...
    BigPoly &BigPoly::operator =(const string &hex_poly)
    {
        ...
        while (pos < length-1)
        {
            ...
            // cout<<"########    2:"<<"power = "<<power<<"last_power = "<<last_power<<endl;
	...
	char* BigPoly::save(int& return_buffer_length)
	{
		int32_t coeff_count32 = static_cast<int32_t>(coeff_count_);
		int32_t coeff_bit_count32 = static_cast<int32_t>(coeff_bit_count_);
		int coeff_uint64_count = divide_round_up(coeff_bit_count_, bits_per_uint64);

		char *buffer=(char*)malloc(2*sizeof(int32_t)+ coeff_count_ * coeff_uint64_count * bytes_per_uint64+1);
		return_buffer_length = 2 * sizeof(int32_t) + coeff_count_ * coeff_uint64_count * bytes_per_uint64 + 1;

		int offset = 0;
		memcpy(buffer+offset, (char*)&coeff_count32, sizeof(int32_t));
		offset += sizeof(int32_t);
		memcpy(buffer+offset, (char*)&coeff_bit_count32, sizeof(int32_t));
		offset += sizeof(int32_t);
		memcpy(buffer+offset, (char*)value_, coeff_count_ * coeff_uint64_count * bytes_per_uint64);
		return buffer;
	}
	...
	void BigPoly::load(char* buffer)
	{
		int offset = 0;
		int32_t read_coeff_count = 0;
		memcpy(&read_coeff_count, buffer + offset, sizeof(int32_t));
		offset += sizeof(int32_t);

		int32_t read_coeff_bit_count = 0;
		memcpy(&read_coeff_bit_count, buffer + offset, sizeof(int32_t));
		offset += sizeof(int32_t);
		...
		if (read_coeff_uint64_count == coeff_uint64_count)
		{
			memcpy(value_, buffer + offset, read_coeff_count * coeff_uint64_count * bytes_per_uint64);
			//stream.read(reinterpret_cast<char*>(value_), read_coeff_count * coeff_uint64_count * bytes_per_uint64);
		}
		else
		{
			...
			for (int i = 0; i < read_coeff_count; ++i)
			{
				memcpy(coeff_ptr,buffer+offset, read_coeff_uint64_count * bytes_per_uint64);
				//stream.read(reinterpret_cast<char*>(coeff_ptr), read_coeff_uint64_count * bytes_per_uint64);
				set_zero_uint(coeff_uint64_count - read_coeff_uint64_count, coeff_ptr + read_coeff_uint64_count);
				coeff_ptr += coeff_uint64_count;
			}
