...
double DecryptForDebug(BigPolyArray input)
{
	...
	double ans = encoder.decode_int32(decryptor.decrypt(input));
	return ans;
}
...
//  cout << "Encoded learningRate" << encLearningRate.to_string() << endl;
 ...
 //   cout<<"-----------------------------------------------------  Encryption(0) finished."<<endl;
		for (int i = 0; i < nFeatures; i++)
		{
//      cout<<"-----------------------------------------------------  Features "<<i<<endl;
			  inputToSigmoid = evaluator.add(evaluator.multiply(XTrain[i], w0[i]), inputToSigmoid);
        inputToSigmoid = DecreaseNoise(inputToSigmoid);
		}
...
//    cout<<"-----------------------------------------------------  decrypto.decrypt begins."<<endl;
		BigPoly ans = decryptor.decrypt(inputToSigmoid);
//    cout<<"-----------------------------------------------------  decrypto.decrypt finished."<<endl;
		crtbuilder.decompose(ans, values);
//    cout<<"-----------------------------------------------------  decompose finished."<<endl;
		...
		cout << "Noise in the inputToSigmoid: " << decryptor.inherent_noise_bits(inputToSigmoid)
			<< "/" << parms.inherent_noise_bits_max() << " bits" << endl;
#endif

		BigPolyArray outputOfSigmoid;
		outputOfSigmoid = sigmod_Hme(inputToSigmoid, numTrainingSamples);
		...
		cout << "Noise in the outOfSigmoid: " << decryptor.inherent_noise_bits(outputOfSigmoid)
			<< "/" << parms.inherent_noise_bits_max() << " bits" << endl;
#endif

		BigPolyArray error;
		error = evaluator.add(outputOfSigmoid, evaluator.negate(yTrain));
		...
		cout << "Noise in the error: " << decryptor.inherent_noise_bits(error)
			<< "/" << parms.inherent_noise_bits_max() << " bits" << endl;
#endif
		...
		for (int i = 0; i < nFeatures; i++)
		{
			...
			cout << "Noise in the (error * trainingSamples) output: " << decryptor.inherent_noise_bits(tmp)
				<< "/" << parms.inherent_noise_bits_max() << " bits" << endl;
#endif

			tmp = AddInRow(tmp, numTrainingSamples);
			wUpdate[i] = tmp;
		}

		for (int i = 0; i < nFeatures; i++)
		{
//      cout << "debug: line 594 in OutEnclaveTest.cpp: " << encLearningRate.to_string() << endl;
			w0[i] = evaluator.add(w0[i], evaluator.multiply_plain(evaluator.negate(wUpdate[i]), encLearningRate));
//      cout << "debug: line 597 in OutEnclaveTest.cpp." << endl;
#ifdef  DEBUG
			ans = decryptor.decrypt(w0[i]);
			crtbuilder.decompose(ans, values);
			cout << "Weight update values (scaled): " << endl;
			for (size_t i = 0; i < numTrainingSamples; ++i)
			{
				cout << "(" << i << ", " << (values[i].to_double()/conf.precision) << ")" << ((i != numTrainingSamples - 1) ? ", " : "\n");
			}
			cout << "Noise in the weight update value: " << decryptor.inherent_noise_bits(w0[i])
				<< "/" << parms.inherent_noise_bits_max() << " bits" << endl;
#endif // ! DEBUG

		}
	}
  cout<<"--------------------------------------    function returns."<<endl;
	return w0;
}
...
//	cout << " Complete Matrix" << endl;
	...
	cout << "**********************************************************************************************" << endl;
#ifdef SHOWRESULT
	cout << "AUC ON TRAINING SET (of size " << trainingSize << ": ";
 //Predict(trainingDataForTesting, trainingYForTesting, encryptWeights, numFeatures, trainingSize, precision);
	...
	cout << endl;
	cout << "AUC ON TESTING SET (of size " << testSize << ": ";
	AUC(Predict(xTest, yTest, encryptWeights, numFeatures, testSize, precision), yTest);
#endif
