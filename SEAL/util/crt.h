...
namespace seal
{
    namespace util
    {
        ...
        public:
            ...
            UIntCRTBuilder(std::vector<const uint64_t*> mods, std::vector<int> mod_uint64_counts);

            ~UIntCRTBuilder()
            {
            }

            void compose(std::vector<const uint64_t*> inputs, uint64_t *destination) const;

            void decompose(const uint64_t *input, std::vector<uint64_t*> destination) const;

            bool generate(std::vector<const uint64_t*> mods, std::vector<int> mod_uint64_counts);
            ...
            inline const uint64_t *get_from_mod_array(int index) const
            {
                return mod_array_[index].get();
            }

            inline const uint64_t *mod_product() const
            {
        ...
        private:
            ...
            std::vector<uint64_t*> mod_products_array_;
            ...
            std::vector<uint64_t*> inv_mod_products_array_;
        };
    }
}