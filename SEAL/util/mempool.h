    ...
    namespace util
    {
        ...
        public:
            MemoryPoolItem(int uint64_count) : pointer_(nullptr), next_(nullptr)
            {
				...
				if (uint64_count > 0) {
					tmp = new uint64_t[uint64_count]; // (uint64_t*)malloc(uint64_count * sizeof(uint64_t));
				}
				pointer_ = tmp;
				//pointer_ = uint64_count > 0 ? (uint64_t*)malloc(uint64_count*sizeof(uint64_t)) : nullptr;
