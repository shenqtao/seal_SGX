...
namespace seal
{
    namespace util
    {
        ...
        public:
            ...
            const uint64_t *get_root() const;

            const uint64_t *get_from_root_powers(int index) const;

            const uint64_t *get_from_scaled_root_powers(int index) const;

            const uint64_t *get_from_inv_root_powers(int index) const;

            const uint64_t *get_from_scaled_inv_root_powers(int index) const;

            const uint64_t *get_from_inv_root_powers_div_two(int index) const;

            const uint64_t *get_from_scaled_inv_root_powers_div_two(int index) const;

            const uint64_t *get_inv_degree_modulo() const;
        ...
        private:
            ...
            void ntt_powers_of_primitive_root(uint64_t *root, MemoryPool &pool, uint64_t *destination);

            // Scales the elements of a vector returned by powers_of_primitive_root(...) by word_size/modulus and rounds down.
            void ntt_scale_powers_of_primitive_root(uint64_t *input, MemoryPool &pool, uint64_t *destination);
