...
namespace seal
{
    ...
    void BigPolyArray::resize(int size, int coeff_count, int coeff_bit_count)
    {
        ...
        // Allocate new space.////////////////////////////////////////////////////////////////////////////////////////////////changed by yichen
		uint64_t *new_value;
		if (size > 0 && coeff_count > 0 && coeff_bit_count > 0)
			new_value = (uint64_t*)malloc(size * coeff_count * new_coeff_uint64_count*sizeof(uint64_t));
		else
			new_value = nullptr;
	...
	char* BigPolyArray::save(int &buffer_size)
	{
		...
		char* buffer = (char*)malloc(3 * sizeof(int32_t) + size_ * coeff_count_ * coeff_uint64_count() * bytes_per_uint64 + 1);
		buffer_size = 3 * sizeof(int32_t) + size_ * coeff_count_ * coeff_uint64_count() * bytes_per_uint64 + 1;

		int offset = 0;
		memcpy(buffer, (char*)&count32, sizeof(int32_t));
		offset += sizeof(int32_t);
		memcpy(buffer + offset, (char*)&coeff_count32, sizeof(int32_t));
		offset += sizeof(int32_t);
		memcpy(buffer + offset, (char*)&coeff_bit_count32, sizeof(int32_t));
		offset += sizeof(int32_t);
		memcpy(buffer + offset, (char*)value_, size_ * coeff_count_ * coeff_uint64_count() * bytes_per_uint64);
		offset += size_ * coeff_count_ * coeff_uint64_count() * bytes_per_uint64;

		return buffer;
	}

	void BigPolyArray::load(char* buffer)
	{
		...
		int offset = 0;
		memcpy(&read_count, buffer + offset, sizeof(int32_t));
		offset += sizeof(int32_t);
		...
		memcpy(value_, buffer + offset, size_ * coeff_count_ * coeff_uint64_count() * bytes_per_uint64);
