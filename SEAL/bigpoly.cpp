...
namespace seal
{
    ...
    void BigPoly::resize(int coeff_count, int coeff_bit_count)
    {
		...
		if (coeff_count > 0 && new_coeff_uint64_count > 0)
			new_value = (uint64_t *)malloc(coeff_count * new_coeff_uint64_count*sizeof(uint64_t));
		else
			new_value = nullptr;
    ...
    BigPoly &BigPoly::operator =(const string &hex_poly)
    {
        ...
        while (pos < length)
        {
            ...
            for (int zero_power = last_power - 1; zero_power > power; --zero_power)
            {
				...
				//if (zero_power == 1)//this is for debug
				//	int i = 0;
            }
	...
	char* BigPoly::save(int &buffer_size)
	{
		...
		buffer_size = 2 * sizeof(int32_t) + coeff_count_ * coeff_uint64_count * bytes_per_uint64 + 1;
